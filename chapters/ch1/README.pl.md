...
- zaindeksować wektory w bazie danych
...
```
Uruchom skrypt `python chapters/ch1/ch1.py` i zobacz co się stanie. A stało się to:
```
...
Wszystko co zrobiliśmy wyżej, jest dokładnie tym co robi baza wektorowa. Dlatego zamiast samemu implementować podobieństwo kosinusowe, embedingi, itp możemy użyć bazy wektorowej, która zrobi to za nas.
...
- `record = db.get(index, include=['documents', 'embeddings'])` - pobieramy dokumenty z bazy wektorowej za pomocą ich indexu. W tym przypadku pobieramy dokumenty i ich wektory. Zwracane są słowniki, które zawierają informacje o dokumentach.
...
embeddingi i zapisuje je w pamięci. Możemy je później pobrać używając ich indexu. Widzimy też, że embeddingi są takie same jak te, które wygenerowaliśmy wcześniej.
...
Ponieważ nasza baza i tak ma dwa dokumnety to nie ma to znaczenia.
...
Jednakże, w obrębach jednej bazy wektorowej własności podoobieństwa są zachowane.
...
Teraz możemy zaindeksować dokumenty w bazie wektorowej:
...
To na tyle jeśli chodzi o etap pierwszy. W kolejnym etapie użyjemu modelu językowego do generowania odpowiedzi na pytania na podstawie znalezionych dokumentów.